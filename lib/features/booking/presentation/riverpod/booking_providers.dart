...
/// Use cases
final initiateBookingProvider =
    Provider((ref) => InitiateBooking(ref.watch(bookingRepositoryProvider)));
final getBookingProvider =
    Provider((ref) => GetBooking(ref.watch(bookingRepositoryProvider)));
final getBookingStatusProvider =
    Provider((ref) => GetBookingStatus(ref.watch(bookingRepositoryProvider)));
final cancelBookingProvider =
    Provider((ref) => CancelBooking(ref.watch(bookingRepositoryProvider)));
...
class BookingFlowState {
  ...
  BookingFlowState copyWith(
          {Booking? booking, bool? isLoading, Failure? error}) =>
      BookingFlowState(
...
class BookingFlowNotifier extends StateNotifier<BookingFlowState> {
  ...
  Future<String?> initiate({required UserInfo user}) async {
    try {
      ...
      final sessionId = ref.read(bookingSessionProvider.notifier).sessionId;

      final booking = await ref
          .read(initiateBookingProvider)
          .call(user: user, cart: cartState.cart, sessionId: sessionId);

      // Persist immediately so unpaid/pending bookings are visible in history.
      ...
      state = state.copyWith(isLoading: false, error: failureFromError(e));
      ErrorReporter.report(
        e,
        stackTrace,
        source: 'booking_flow.initiate',
      );
      return null;
    }
  }
...
final paymentStatusProvider =
    StreamProvider.family<Booking, String>((ref, bookingId) async* {
