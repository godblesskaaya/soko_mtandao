    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => const [
              PopupMenuItem(value: 'start_date', child: Text('Sort: Start Date')),
              PopupMenuItem(value: 'end_date', child: Text('Sort: End Date')),
              PopupMenuItem(value: 'created_at', child: Text('Sort: Created At')),
            ],
          ),
          IconButton(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          ...
          try {
            await ref.read(bookingListCombinedProvider(_query).future).timeout(const Duration(seconds: 8));
          } catch (_) {}
        },
          ...
          data: (bookingList) {
            if (bookingList.isEmpty) {
              return ListView(
                ...
                children: [
                  const SizedBox(height: 200),
                  Center(child: Text(_page > 1 ? "No more bookings." : "No bookings yet.")),
                  if (_page > 1)
                    Center(
              ...
              itemBuilder: (context, i) {
                ...
                return Card(
                  child: ListTile(
                    ...
                    onTap: () async {
                      await context.push("/manager/bookings/${data.detail.id}");
                      ref.invalidate(bookingListCombinedProvider(_query));
                    },
                  ),
                );
