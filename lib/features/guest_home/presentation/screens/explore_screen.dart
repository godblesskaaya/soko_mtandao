...
import 'package:soko_mtandao/features/explore/presentation/riverpod/map_state.dart'
    as custom_map_state;
...
class _ExploreMapScreenState extends ConsumerState<ExploreMapScreen> {
    ...
    _debounce = Timer(const Duration(milliseconds: 320), () async {
      ...
      final bounds = await _map!.cameraForCoordinateBounds(
        /* Mbx */ CoordinateBounds(
          southwest: Point(
              coordinates: Position(camera.center.coordinates.lng - 0.05,
                  camera.center.coordinates.lat - 0.05)),
          northeast: Point(
              coordinates: Position(camera.center.coordinates.lng + 0.05,
                  camera.center.coordinates.lat + 0.05)),
      ...
      ref.read(custom_map_state.cameraStateProvider.notifier).state =
          custom_map_state.CameraState(
        lat: lat.toDouble(),
        lng: lng.toDouble(),
        zoom: zoom,
        bounds: b,
  ...
  Future<void> _initPins(List<Hotel> hotels) async {
    ...
    final Uint8List icon = await rootBundle
        .load("assets/icons/hotel_icon_blue.png")
        .then((value) => value.buffer.asUint8List());
    final Uint8List highlightedIcon = await rootBundle
        .load("assets/icons/hotel_icon_yellow.png")
        .then((value) => value.buffer.asUint8List());

    _pointAnnotationTapSubscription ??=
        _pinManager!.tapEvents(onTap: (PointAnnotation ClickedAnnotation) {
      final String? annotationId = ClickedAnnotation.id;
      if (annotationId != null) {
        final hotelId = annotationToHotelId[annotationId];

        if (hotelId != null) {
          ref.read(custom_map_state.selectedHotelIdProvider.notifier).state =
              hotelId;
  ...
  Future<void> _moveToInitial() async {
    final init =
        await ref.read(custom_map_state.initialLocationProvider.future);
        ...
        children: [
              ...
              child: Column(
                ...
                children: [
                  _SearchBar(
                    initial: searchQuery,
                    onChanged: (v) => ref
                        .read(custom_map_state
                            .exploreSearchQueryProvider.notifier)
                        .state = v,
                    onClear: () => ref
                        .read(custom_map_state
                            .exploreSearchQueryProvider.notifier)
                  ...
                  Align(
                    ...
                    child: FilledButton.tonalIcon(
                      onPressed: () => context.goNamed('hotels'),
                      icon: const Icon(Icons.list_alt),
                      label: const Text('Browse Full Hotel List'),
                    ),
                  ),
                ],
              ),
          ...
          Consumer(
            builder: (_, r, __) {
              ...
              final tooLow = cam.zoom < MapConfig.minZoomForData &&
                  (r.read(custom_map_state.exploreSearchQueryProvider).isEmpty);
              ...
              return Align(
                ...
                child: SafeArea(
                  child: Container(
                    ...
                    child: const Text('Zoom in to explore hotels',
                      ...
                      child: hotelsAsync.when(
                        ...
                        data: (hotels) {
                          ...
                          WidgetsBinding.instance
                              .addPostFrameCallback((_) => _initPins(hotels));

                          if (hotels.isEmpty) {
                            return const Center(
                                child: Text('No hotels here yet.'));
                          }
                            ...
                            itemBuilder: (_, i) {
                              ...
                              return HotelListItem(
                                ...
                                onTap: () {
                                  ref
                                      .read(custom_map_state
                                          .selectedHotelIdProvider.notifier)
                                      .state = h.id;
                                  setState(() => _highlightedId = h.id);
...
class _SearchBar extends StatefulWidget {
  ...
  const _SearchBar(
      {required this.initial, required this.onChanged, required this.onClear});
